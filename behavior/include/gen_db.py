...
def main():
	try:
		...
		while True:
			r = raw_input("There is a existing poses data base with {0} items. Do you want to overwrite it?(y/n)".format(poses.shape[0]))
			if r.lower() == 'y':
	...
	try:
		...
		while True:
			r = raw_input("There is a existing buttons data base with {0} items. Do you want to overwrite it?(y/n)".format(buttons.shape[0]))
			if r.lower() == 'y':
